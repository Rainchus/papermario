...
PageInfo mainPage = {
    .entries = {
        .entryText = {
            //x0, x1, (null)
            {testTextMainMenu, 0, 0, 0, 0, 0, 0, 0},
            //y0, y1, (null)
            {testTextMainMenu2, 0, 0, 0, 0, 0, 0, 0} },
        .textPositions = {
...
PageInfo pageIndex1 = {
    .entries = {
        .entryText = {
            {testTextMainMenu3, 0, 0, 0, 0, 0, 0, 0}, 
            {testTextMainMenu4, 0, 0, 0, 0, 0, 0, 0}},
        .textPositions = {
            {0, 0, 0, 0, 0, 0, 0, 0}, //x Positions
            {0, 12, 24, 36, 48, 60, 72, 84} // Y positions
        }, 
...
void drawDebugMenuText(BoxInfo debugBox) { //debugBox defines root position
    ...
    for (i = 0; pages[debugCurMenu]->entries.entryText[debugCurMenu][i] != 0; i++) {
        for (j = 0; pages[debugCurMenu]->entries.entryText[i][j] != 0; j++) {
            ...
            xPos = debugBox.xPos + 8 + pages[debugCurMenu]->entries.textPositions[j][0];
            yPos = debugBox.yPos + 4 + pages[debugCurMenu]->entries.textPositions[j][1];
            ...
            convertAsciiToMsg((areaBufferAscii + sizeof(msgCommandsBuffer) -1),
                                pages[debugCurMenu]->entries.entryText[i][j]);
...
void mod_main_func(void) {
    ...
    if (gGameStatus.currentButtons[0] & BUTTON_R && gGameStatus.pressedButtons[0] & BUTTON_D_RIGHT ) {
        ...
        if (debugCurMenu >= ARRAY_COUNT(pages)) {
            debugCurMenu = ARRAY_COUNT(pages) - 1;
        }
